...
function hasFoodItemContent(item) {
  ...
  return Boolean(
    item.nombre?.trim() || item.descripcion?.trim() || item.imagen?.trim() || item.tipo?.trim() || (item.calorias ?? 0) > 0
  );
}
function slotHasContent(slot) {
  return hasFoodItemContent(slot.platilloPrincipal) || hasFoodItemContent(slot.guarnicion1) || hasFoodItemContent(slot.guarnicion2) || Boolean(slot.contenedor?.trim()) || slot.adicionales.length > 0;
}
...
export { DAY_OF_WEEK_VALUES as D, foodCatalogItemInputSchema as f, weeklyMenuInputSchema as w };
