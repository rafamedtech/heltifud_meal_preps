...
import type { Expense, ExpenseCategory, ExpenseInput, ExpenseSummary } from "~~/layers/menu/shared/types/types"
...
const summary = ref<ExpenseSummary>({ total: 0, count: 0, average: 0, currentMonthTotal: 0, previousMonthTotal: 0 })
const monthlyChange = computed(() =>
  expenseChangePercent(summary.value.currentMonthTotal, summary.value.previousMonthTotal)
)
const monthlyChangeLabel = computed(() =>
  monthlyChange.value === null
    ? "Sin base"
    : new Intl.NumberFormat("es-MX", { style: "percent", maximumFractionDigits: 1, signDisplay: "exceptZero" }).format(
        monthlyChange.value / 100
      )
)
...
const hasFormChanges = computed(() =>
  (Object.keys(initialFormState.value) as (keyof ExpenseInput)[]).some(
    (key) => key !== "description" && formState[key] !== initialFormState.value[key]
  )
)
...
const activeAdvancedFilters = computed(
  () =>
    [
      ...
      selectedExpenseType.value !== "todos",
      selectedInvoiceStatus.value !== "todos",
      Boolean(fromDate.value || toDate.value)
...
function createVendor(value: string) {
  ...
  const existing = vendorOptions.value.find(
    (option) => option.localeCompare(vendor, "es", { sensitivity: "base" }) === 0
  )
  formState.vendor = existing ?? vendor

  if (!existing) {
    vendorOptions.value = [...vendorOptions.value, vendor].sort((first, second) => first.localeCompare(second, "es"))
  }
}
...
async function confirmDelete() {
  ...
  try {
    ...
    if (editingExpense.value?.id === deleted.id) {
      isFormOpen.value = false
      isCancelConfirmOpen.value = false
      editingExpense.value = null
    }
    ...
    toast.add({
      ...
      description: `${deleted.description} fue eliminado.`,
      color: "success",
      icon: "i-lucide-check-circle"
    })
    await loadExpenses()
...
function updateExpenseDescription() {
  formState.description = buildExpenseDescription(formState.category, formState.vendor, formState.expenseDate)
}
...
watch([selectedCategory, selectedExpenseType, selectedInvoiceStatus, fromDate, toDate], () => loadExpenses())
watch(search, () => {
  clearTimeout(searchTimer)
  searchTimer = setTimeout(() => loadExpenses(), 300)
})
watch([() => formState.category, () => formState.vendor, () => formState.expenseDate], updateExpenseDescription, {
  immediate: true
})
...
<template>
  <main class="flex min-h-full flex-col space-y-6">
    <section class="grid gap-3 sm:grid-cols-3">
      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div>
            <p class="text-xs font-medium uppercase tracking-wider text-muted">Vs. mes anterior</p>
            <USkeleton
              v-if="loading"
              class="mt-2 h-8 w-28"
            />
            <p
              v-else
              class="mt-2 text-2xl font-semibold text-highlighted"
              :title="
                monthlyChange === null
                  ? 'El mes anterior no tuvo gastos para calcular la variación.'
                  : 'Variación de los gastos de este mes respecto al mes anterior.'
              "
            >
              {{ monthlyChangeLabel }}
            </p>
      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div>
            <p class="text-xs font-medium uppercase tracking-wider text-muted">Este mes</p>
            <USkeleton
              v-if="loading"
              class="mt-2 h-8 w-28"
            />
            <p
              ...
              class="mt-2 text-2xl font-semibold text-highlighted"
            >
              {{ currency(summary.currentMonthTotal) }}
            </p>
          </div>
      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div>
            <p class="text-xs font-medium uppercase tracking-wider text-muted">Movimientos</p>
            <USkeleton
              v-if="loading"
              class="mt-2 h-8 w-16"
            />
    ...
    <UCard
      ...
      :ui="{ body: 'p-5 sm:p-6' }"
    >
      <div class="grid gap-3 min-[744px]:grid-cols-2">
        <UInput
          v-model="search"
          icon="i-lucide-search"
          placeholder="Buscar gasto, proveedor o referencia"
          class="min-w-0 w-full"
        />
        <div
          class="grid min-w-0 gap-3 min-[744px]:landscape:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] min-[1025px]:portrait:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]"
        >
          <div class="hidden min-w-0 min-[744px]:landscape:block min-[1025px]:portrait:block">
            <CalendarRangeInput
          ...
          <UButton
            ...
            size="xl"
            :label="activeAdvancedFilters ? `Más filtros (${activeAdvancedFilters})` : 'Más filtros'"
    ...
    >
      <div class="mx-auto max-w-md space-y-4 text-center">
        ...
        />
        <div>
          <h2 class="font-semibold text-highlighted">No fue posible cargar los gastos</h2>
    ...
    >
      <div class="mx-auto max-w-md space-y-4 text-center">
        <div class="mx-auto flex size-12 items-center justify-center rounded-xl border border-default bg-elevated">
          <UIcon
            :name="isFiltering ? 'i-lucide-search-x' : 'i-lucide-receipt-text'"
            class="size-5 text-muted"
          />
        </div>
        <div>
          <h2 class="font-semibold text-highlighted">
            {{ isFiltering ? "No encontramos gastos" : "Aún no hay gastos" }}
          </h2>
          <p class="mt-1 text-sm text-muted">
            {{
              isFiltering
                ? "Ajusta la búsqueda o limpia los filtros."
                : "Registra el primer movimiento para comenzar el control."
            }}
          </p>
        </div>
        <UButton
          ...
          @click="resetFilters"
          >Limpiar filtros</UButton
        >
        <UButton
          v-else
          icon="i-lucide-plus"
          @click="openCreate"
          >Registrar gasto</UButton
        >
      </div>
    </UCard>
    ...
    <template v-else>
      <section class="grid gap-3 min-[744px]:landscape:hidden min-[1025px]:portrait:hidden">
        <UCard
          ...
          class="app-surface cursor-pointer transition-colors hover:bg-elevated/50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
          :ui="{ body: 'p-5 sm:p-5' }"
          @click="openEdit(expense)"
          @keydown.enter.prevent="openEdit(expense)"
          @keydown.space.prevent="openEdit(expense)"
        >
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <p class="mb-1 text-sm text-muted">{{ displayDate(expense.expenseDate) }}</p>
              <div class="flex items-center gap-2">
                <UIcon
                  :name="categoryIcon(expense.category)"
                  class="size-4 shrink-0 text-primary"
                />
                <h2 class="truncate font-semibold text-highlighted">{{ expense.description }}</h2>
              </div>
              <p
                v-if="expense.billingReference1 || expense.billingReference2"
                class="mt-3 truncate text-xs text-muted"
              >
          ...
          <div class="mt-5 flex items-end justify-between gap-4 border-t border-default/70 pt-4">
            <UBadge
              :color="expense.isInvoiced ? 'success' : 'warning'"
              variant="soft"
            >
              <UIcon
                :name="expense.isInvoiced ? 'i-lucide-file-check-2' : 'i-lucide-file-clock'"
                class="size-3"
              />
      ...
      <UCard
        class="app-surface hidden overflow-hidden min-[744px]:landscape:block min-[1025px]:portrait:block 2xl:hidden"
        :ui="{ body: 'p-0 sm:p-0' }"
      >
        <UTable
          :data="expenses"
        ...
        >
          <template #description-cell="{ row }">
            <div class="flex min-w-0 items-center gap-3">
              <div class="flex size-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
                <UIcon
                  :name="categoryIcon(row.original.category)"
                  class="size-4"
                />
              </div>
              <div class="min-w-0">
                <p class="wrap-break-word font-semibold leading-snug text-highlighted">
          ...
          <template #details-cell="{ row }">
            <div class="flex max-w-48 flex-wrap gap-1.5">
              <UBadge
                color="neutral"
                variant="soft"
                >{{ optionLabel(categoryOptions, row.original.category) }}</UBadge
              >
            </div>
          </template>
          <template #billingStatus-cell="{ row }">
            <UBadge
              :color="row.original.isInvoiced ? 'success' : 'warning'"
              variant="soft"
            >
              <UIcon
                :name="row.original.isInvoiced ? 'i-lucide-file-check-2' : 'i-lucide-file-clock'"
                class="size-3"
              />
          ...
          <template #amount-cell="{ row }"
            ><span class="whitespace-nowrap font-semibold text-highlighted">{{
      ...
      <UCard
        class="app-surface hidden overflow-hidden 2xl:block"
        :ui="{ body: 'p-0 sm:p-0' }"
      >
        <UTable
          :data="expenses"
        ...
        >
            ...
            <div class="space-y-1.5">
              <UBadge
                ...
                ><UIcon
                  :name="row.original.isInvoiced ? 'i-lucide-file-check-2' : 'i-lucide-file-clock'"
                  class="size-3"
                />{{ row.original.isInvoiced ? "Facturado" : "Pendiente" }}</UBadge
              >
              <p
                v-if="row.original.billingReference1 || row.original.billingReference2"
                class="max-w-44 truncate text-xs text-muted"
              >
                {{ [row.original.billingReference1, row.original.billingReference2].filter(Boolean).join(" · ") }}
              </p>
              <p
                v-else
          ...
          <template #amount-cell="{ row }"
            ><span class="font-semibold text-highlighted">{{ currency(row.original.amount) }}</span></template
          >
        </UTable>
      </UCard>
      ...
      <template #body>
        <div class="space-y-5">
          <UFormField
            ...
            class="min-[744px]:landscape:hidden min-[1025px]:portrait:hidden"
          >
            <CalendarRangeInput
              v-model:start="draftFromDate"
              v-model:end="draftToDate"
              size="xl"
              class="w-full"
            />
          </UFormField>
        ...
        >
          <div class="grid gap-5 sm:grid-cols-2">
            ...
            <UFormField
              ...
              required
              ><USelect
                v-model="formState.expenseType"
                :items="expenseTypeOptions.slice(1)"
                value-key="value"
                class="w-full"
            /></UFormField>
            <UFormField
            ...
            />
            <UFormField
              label="Referencia de facturación 1"
              name="billingReference1"
              hint="Opcional"
              ><UInput
                v-model="formState.billingReference1"
                icon="i-lucide-hash"
                placeholder="Folio u orden de compra"
                class="w-full"
            /></UFormField>
            <UFormField
              label="Referencia de facturación 2"
              name="billingReference2"
              hint="Opcional"
              ><UInput
                v-model="formState.billingReference2"
                icon="i-lucide-fingerprint"
                placeholder="UUID, ticket u otra referencia"
                class="w-full"
            /></UFormField>
            <UFormField
            ...
            >
              <USwitch
                v-model="formState.isInvoiced"
                size="xl"
                :ui="{ label: 'font-normal text-highlighted' }"
                label="Este gasto ya fue facturado"
                description="Actívalo cuando el comprobante fiscal esté disponible."
              />
            </UFormField>
            <UFormField
              label="Notas"
              ...
              ><UTextarea
                ...
                placeholder="Detalles, folio o contexto adicional"
                size="xl"
                autoresize
                :rows="3"
                class="w-full"
          ...
          <div class="flex flex-col gap-2 sm:flex-row sm:justify-end">
            <UButton
              ...
              @click="pendingDelete = editingExpense"
            >
              Eliminar gasto
            </UButton>
            <UButton
            ...
            <UButton
              ...
              icon="i-lucide-save"
              :loading="saving"
              >{{ editingExpense ? "Guardar cambios" : "Registrar gasto" }}</UButton
            >
          </div>
        </UForm>
      </template>
      ...
      <template #footer>
        <div class="flex w-full flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <UButton
            ...
            >Seguir editando</UButton
          >
          <UButton
            color="error"
            @click="discardForm"
            >Descartar cambios</UButton
          >
        </div>
      </template>
    </UModal>
    ...
    <UModal
      ...
      :description="pendingDelete ? `Se eliminará ${pendingDelete.description}.` : undefined"
      :ui="{ content: 'max-w-md' }"
    >
      <template #body
        ><UAlert
          ...
          description="El movimiento dejará de aparecer en el historial y en los totales."
      /></template>
      <template #footer
